...
| VE6 | `apps/api/src/seed.ts:202,210,218,226,249` seed `accountStatus: 'active'` for all five accounts; `suspended` and `archived` are never seeded | The E2E observers pin **one** domain member. That is sufficient for the render path, because the render site is a single member-agnostic expression — exercising one member exercises it for all three. What stays member-specific is the key→copy mapping, which I6.2/I6.3 and `apps/web/test/i18n.test.ts:49` cover on the unit tier. Seeding a fourth state is rejected under RT2: it would join the orphan set derived at `e2e/fixtures/seed-facts.ts:59-61`, red `accounts.spec.ts`'s by-name orphan assertions, and force a matching edit to `e2e/scripts/assert-seed-preserved.sh` policed by `apps/api/test/seed-gate-agreement.test.ts:90`. That is a seed-design change, not an observer. |
...
its inverse, and the re-seed as fallback, scoped `WHERE id = '<accountId>'`, **run through the
session `docs/manual-tests/e2e-howto.md:50-51` already uses** (`docker compose exec postgres psql
-U postgres …`). Naming the session is not pedantry: `saas_accounts` carries
