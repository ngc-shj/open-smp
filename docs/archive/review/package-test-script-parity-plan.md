...
- **SC58** — **CLOSED, and it was closed before this entry was last read.** The
  three-line pin lives in `package-test-parity.test.ts` and asserts
  `eslint.config.mjs`'s default-exported `ignores` array by exact equality
  through the live-import idiom C6 uses. Verified in cycle 8 by reading the
  code rather than the entry: the hazard it names — `ignores: ['**/*']` exits 2
  (loud) while `ignores: ['apps/api/**']` exits 0 with planted errors silently
  absent — is guarded. What this entry records now is the *second* defect, which
  is its own: a scope item that says "deferred" while the work is done sends the
  next cycle to re-do it, and nothing observes a scope item. Cycle 8 spent a
  measurement discovering the deferral was stale.
- **SC59** — **CLOSED in cycle 8**, at the trigger this entry named ("the next
  cycle touching root tooling — and it should be closed there rather than
  deferred again"). A root `tsconfig.json` with `include: ["*.ts"]` now exists
  and `typecheck` is `tsc --noEmit -p tsconfig.json && pnpm -r --parallel exec
  tsc --noEmit`; C8's pin was updated in the same commit, so the value is
  gate-enforced. **Red-proven**: a planted `TS2769` in `vitest.config.ts` takes
  `pnpm typecheck` from exit 0 to exit 2, where before it was invisible —
  `pnpm -r exec` excludes the workspace root (measured: 11 member directories,
  none of them the root), so there was no root program at all. The entry's own
  refutations held: the gate's live import cannot observe a type error, because
  Vite erases types at transform time; and SC50 forbids changing project
  definitions and job structure, which a root program plus a script edit is
  not.
