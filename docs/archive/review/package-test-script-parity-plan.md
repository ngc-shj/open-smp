...
The three are locked at revision 9, with every assertion red-proven against the shipped tree. One difference from C1–C8 is stated rather than glossed: those eight were reviewed before implementation and have taken **zero** Major findings across four code-review rounds. C9–C11 were implemented before they were contracted and have taken **twelve Major and three Critical across two reviews**. The count of rounds it took is the part worth carrying to the next cycle.
...
**50 mutations executed against the shipped tree in revision 9** — 38 red-proofs and 12 allow-side controls. Counts are generated from the rows below, because revision 8 stated four different totals across the plan, the deviation log and two commit messages, none of which matched its own table. The harness runs serially, asserts each anchor before writing, reads every verdict from a captured exit status, and restores from the git index with a byte comparison afterwards.

**Four of the fifty were mis-specified on the first run**, which is the more useful record. Three were harness errors — a deny probe whose failure had the *same* cause it was meant to differ from, a family narrowing caught by an earlier self-test rather than the assertion it targeted, and a guard inversion that filled the parsed set with garbage instead of emptying it. The fourth was not: re-adding a `docs/` directory anchor stayed **green** against the repaired exclusion guard, proving the repair insufficient and forcing the predicate-level assertion that shipped. A mutation set that never surprises its author is not testing anything.
...
| MT16 | the exclusions guard: re-add a docs/ directory anchor | exit 1 — `a non-markdown file under docs/ is excluded from the scan` |
...
| MC14 | the Dockerfile pnpm version drifts from packageManager | exit 1 — `the Dockerfile does not install exactly `pnpm@10.34.5`; the pinned lockfile would be read by an unpinned tool` |

**Allow side (RT10) — every one must stay GREEN, and all 12 did.**
...
Five deny shapes and four allow shapes are additionally asserted **inside** the gate against synthetic input, because C9's expected set is empty and a predicate matching nothing would otherwise pass everything. M-T12 proves that self-test fires.

**Not falsifiability mutations**: `docker build --target deps` and the in-image probes (SC62, manual step 4/4b) are positive controls — they must stay green, and they are the only image-level observations this cycle makes.
