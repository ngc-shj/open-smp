...
**63 mutations executed against the shipped tree in revision 10** — 49 red-proofs and 14 allow-side controls, all counts generated from the rows. The harness runs serially, asserts each anchor before writing, reads every verdict from a captured exit status, and restores from the git index with a byte comparison afterwards.

Two mis-specifications this round, both instructive. **MT21** stayed green under a mutation that gutted the off-switch predicate, because the self-test cell it was meant to prove carried a `--filter` and passed through `SELECTOR_FAMILY` instead — the round-5 review had raised exactly this shape about the round-4 cells, and the first draft of the round-5 cells reproduced it. The cells are selector-free now. **MC19** expected a red from moving `WORKDIR`; it is green **by design** once the workdir is derived rather than typed, because every deps-stage destination is relative and never consults it. Re-specified to mutate the derivation itself, with the WORKDIR move kept as its positive control (MA12).
...
| MT23 | pnpm matched by exact token again, not basename | exit 1 — `the predicate no longer detects pnpm invoked by path` |
...
The gate additionally asserts its predicates against synthetic input before scanning — C9 runs `selectorLines` over 14 deny shapes and 4 allow shapes, `isExcluded` over 5, and C10 exercises `DEPS_STAGE`, `ANY_STAGE` and `DEPS_INSTALL` in both directions. Those counts are read off the code, not carried in prose: revision 9 stated them twice, differently, and both were wrong.
