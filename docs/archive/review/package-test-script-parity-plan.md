...
- **SC58** *(code review round 1)* — `pnpm lint`'s effective file set is observed by nothing. C8 pins the string `eslint .`, but measured: `ignores: ['**/*']` exits 2 (loud) while `ignores: ['apps/api/**']` exits **0** with planted errors silently absent — the same total-loud / partial-silent asymmetry the plan documents for Playwright's `testIgnore`. Low today because `eslint.config.mjs` carries no security rules; it rises the moment one is added. Closing it needs an eslint-scope reconciliation control roughly the size of control 5. Trigger: adding a security rule to the ESLint config.
- **SC59** *(code review round 1)* — `pnpm typecheck` is `pnpm -r --parallel exec tsc --noEmit`, and `pnpm -r exec` **excludes the workspace root** (verified: 11 directories, none of them the root). There is no root `tsconfig.json`, so `vitest.config.ts` — which this cycle rewrites and the gate live-imports — is typechecked by nothing. Bounded: the live import means a *runtime* error there reds `pnpm test:unit`. Closing it needs a root `tsconfig.json` plus a root typecheck step, which changes CI job structure (SC50). Trigger: the next cycle that touches CI job structure.
