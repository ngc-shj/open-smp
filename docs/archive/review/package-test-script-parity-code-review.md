...
| **S11** | **Required status checks are 403 on this repository.** `branches/main/protection` and `rulesets` both return `Upgrade to GitHub Pro or make this repository public`. SC56 deferred the out-of-band observer as a cost decision; it is structurally unavailable. **A red check does not block a merge here.** *(True when round 7 measured it; superseded at the start of cycle 7 — the repository is public and `main` requires all three checks. Annotated rather than rewritten, because a round record states what that round found, and because an unqualified present-tense claim about the world is exactly what SC65 records being quoted as evidence after it stopped being true.)* | C | Major |
