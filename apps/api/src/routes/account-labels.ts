...
import type { AccountLabelKind, AccountLabelResponse } from '@open-smp/api-types';
...
import { recordLabelAudit } from '../audit.js';
  ...
  app.put(
    ...
    async (req, reply) => {
      ...
      const result = await withTenant(deps.pool, tenantId, async (tx) => {
        ...
        const prior = await tx.query<{ kind: AccountLabelKind; note: string | null }>(
        ...
        const upserted = await tx.query<{ kind: AccountLabelKind; note: string | null }>(
        ...
        await recordLabelAudit(tx, tenantId, 'label_set', {
          actorUserId: userId,
          saasAccountId,
          before: priorRow ? { kind: priorRow.kind, note: priorRow.note } : null,
          after: { kind: row.kind, note: row.note },
        });
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      const found = await withTenant(deps.pool, tenantId, async (tx) => {
        ...
        const deleted = await tx.query<{ kind: AccountLabelKind; note: string | null }>(
        ...
        if (removed) {
          await recordLabelAudit(tx, tenantId, 'label_cleared', {
            actorUserId: userId,
            saasAccountId,
            before: { kind: removed.kind, note: removed.note },
            after: null,
          });
        }
